...
class CFG:
    # File paths
    BASE_PATH = Path('./data')
    TRAIN_FEATURES_PATH = BASE_PATH / 'train_features_refined.csv'
...
required_files = [CFG.TRAIN_FEATURES_PATH, CFG.TEST_FEATURES_PATH]
...
train_features_df_ = pd.read_csv(CFG.TRAIN_FEATURES_PATH)

train_features_df = train_features_df_[ ['id', 'filepath', 'site'] ]
...
train_labels_df = train_features_df_[ ['id', 'label'] ]
...
# test_features_df = test_features_df.sample(n=100)
...
for fold in range(0, CFG.N_FOLDS):
    ...
    set_seed(int(str(CFG.SEED) + str(fold)), reproducible=True)
    ...
    # try:
    #     lr_min, lr_steep = learn.lr_find(suggest_funcs=SuggestionMethod.Steep)
    ...
    logits, _ = learn.get_preds(dl=test_dl, act=lambda x: x)
...
with open("fast_ai_k_fold_post_refine_data.pkl", "wb") as f:
    pickle.dump({
...
# Save submission
submission_df.to_csv('submission_refined_fast_ai_k_fold_accuracy.csv', index=False)
...
# Save submission
submission_df.to_csv('submission_refined_fast_ai_k_fold_loss.csv', index=False)
